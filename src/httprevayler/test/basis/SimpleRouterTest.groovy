...
class SimpleRouterTest {
	...
	void shouldCallAttachedResource() {
		...
		assert router.run(new SimpleRequest(uri:"/fooz")) == FooResource.class
	}
	...
	void shouldCallRightResourceByMissing() {
		...
		assert router.run(new SimpleRequest(uri:"/foo")) == FooResource.class
	}
	
	@Test
	void shouldParseQueryStringParams() {
		def router = new SimpleRouterDummy()
		def request = new SimpleRequest(uri:"/fooz/3/5")
